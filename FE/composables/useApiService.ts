...
export const useApiService = () => {
  ...
  const auth = {
    login: (credentials: LoginCredentials) =>
      post<ApiResponse<{ user: User; token: string }>>(
        "/auth/login",
        credentials,
      ),
    ...
    getProfile: () => get<ApiResponse<User>>("/auth/profile"),
  ...
  const batteries = {
    ...
    getById: (id: string) => get<ApiResponse<Battery>>(`/batteries/${id}`),

    create: (
      data: Omit<
        Battery,
        "id" | "sellerId" | "seller" | "createdAt" | "updatedAt"
      >,
    ...
    delete: (id: string) => del<ApiResponse>(`/batteries/${id}`),

    search: (query: string) =>
      get<PaginatedResponse<Battery>>(
        `/batteries/search?q=${encodeURIComponent(query)}`,
      ),
  };
  ...
  const vehicles = {
    ...
    getById: (id: string) => get<ApiResponse<Vehicle>>(`/vehicles/${id}`),
    ...
    delete: (id: string) => del<ApiResponse>(`/vehicles/${id}`),

    search: (query: string) =>
      get<PaginatedResponse<Vehicle>>(
  ...
  const auctions = {
    ...
    getById: (id: string) => get<ApiResponse<Auction>>(`/auctions/${id}`),
    ...
    delete: (id: string) => del<ApiResponse>(`/auctions/${id}`),

    placeBid: (auctionId: string, amount: number) =>
      post<ApiResponse>(`/auctions/${auctionId}/bid`, { amount }),

    getActive: () => get<PaginatedResponse<Auction>>("/auctions?status=active"),
  ...
  const accessories = {
    getAll: (params?: PaginationParams) => {
      const query = params
        ? `?${new URLSearchParams(params as Record<string, string>).toString()}`
        : "";
      return get<PaginatedResponse<Accessory>>(`/accessories${query}`);
    },

    getById: (id: string) => get<ApiResponse<Accessory>>(`/accessories/${id}`),
    ...
    create: (
      data: Omit<
        ...
        "id" | "sellerId" | "seller" | "createdAt" | "updatedAt"
      >,
    ) => post<ApiResponse<Accessory>>("/accessories", data),

    update: (id: string, data: Partial<Accessory>) =>
      patch<ApiResponse<Accessory>>(`/accessories/${id}`, data),

    delete: (id: string) => del<ApiResponse>(`/accessories/${id}`),
  };
